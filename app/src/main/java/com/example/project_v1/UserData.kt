...
data class UserData(var email: String = "", var uid: String = "", var name: String = "") : Serializable {
    ...
    val badgeList: MutableList<String> = mutableListOf()//뱃지 데이터, 기본적으로 얻은 순서대로 저장
    fun addBadge(badge:String) { //뱃지 추가
    ...
    fun badgeListsortbyname():MutableList<String>{ //뱃지 리스트를 이름순으로 정렬해서 리턴
        var list = badgeList
        list.sort()
        return list
    }
