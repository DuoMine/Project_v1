...
class MainActivity : AppCompatActivity() {
    ...
    fun reloadTodo(linearLayout: LinearLayout) { //todo프래그먼트에 있는 리니어레이아웃 안의 todoInnerBox객체들을 리로드
        ...
        for (item in userData.todoList) {
            ...
            inflatedLayout.findViewById<TextView>(R.id.todoInnerTimeTextView).setText(item.year.toString()+"/"+item.month.toString()+"/"+item.day.toString())
    ...
    fun reloadHabit(linearLayout: LinearLayout){
        ...
        for (item in userData.habitList) {
            ...
            var str = ""
            if(item.dayCheck){
            inflatedLayout.findViewById<TextView>(R.id.habitInnerDayTextView).text = "매일"
            }else{
                if(item.sunCheck)
                    str += " 일"
                if(item.monCheck)
                ...
                if(item.satCheck)
                    str += " 토"
                inflatedLayout.findViewById<TextView>(R.id.habitInnerDayTextView).text = str
            }
    ...
    private fun habitOnClick(v: View, habitInnerBox: HabitInnerBox) {
        ...
        when(v.id){
            R.id.habitboxLinearLayout -> { //박스를 터치 시 HabitInnerBox 확인 창을 띄움
                val mfragment = FragmentModifyHabitDialog()
                val bundle = Bundle()
    ...
    fun reloadProgress(){
        ...
        findViewById<ProgressBar>(R.id.progressBar).progress = num * 100 / userData.habitList.size
        findViewById<TextView>(R.id.progressTextView).text = (num *100 / userData.habitList.size).toString() + "%"
    }
}