...
class FragmentSetBadgeDialog : DialogFragment(), View.OnClickListener {
    ...
    override fun onClick(v: View?){
        when(v?.id){
            R.id.badgeChangeBtn -> {
                for(item in (activity as BadgeActivity).userData.badgeList){
                    if(item.badgeName == binding.badgeNameTextView.text){
                        (activity as BadgeActivity).userData.equippedBadge = item
                        FirebaseDatabase.getInstance().reference.child("Users").child((activity as BadgeActivity).userData.uid).child("equippedBadge").setValue(item)
                    }
                }
            }
